...
Run `npm run build` to check TypeScript and the production bundle. The current repository deploy workflow publishes only the backend; a public frontend deployment needs static hosting configured to serve `frontend/dist` and to route or allow requests to the API. A production build for a separate frontend origin needs `VITE_API_BASE_URL=https://api.helpmake-id.live npm run build` and that origin in backend CORS.
