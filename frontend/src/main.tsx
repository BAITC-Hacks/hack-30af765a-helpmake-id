...
import { compareScenarios, createScenario, explain, getData, getScenario, simulate } from './api'
import { ResultEvidence } from './ResultEvidence'
import { RecommendationPanel } from './RecommendationPanel'
import { ScenarioComparison } from './ScenarioComparison'
import { ExperienceLayer } from './ExperienceLayer'
import { forDataset, loadComparison, loadServerRefs, refsForDataset, saveComparison, saveServerRefs } from './comparison'
import type { ComparisonSlots, ServerSlotRefs, Slot } from './comparison'
import type { Advisor, CriticalPair, DataSet, Decision, District, Measure, Quarter, ServerScenarioComparison, SimulationSuccess } from './types'
import './styles.css'
...
function App() {
  ...
  const [serverRefs, setServerRefs] = useState<ServerSlotRefs>(loadServerRefs)
  const [serverComparison, setServerComparison] = useState<ServerScenarioComparison | null>(null)
  ...
  useEffect(() => { saveServerRefs(serverRefs) }, [serverRefs])
  useEffect(() => { window.scrollTo(0, 0) }, [page])
  useEffect(() => {
    if (page !== 'compare' || !serverRefs.A || !serverRefs.B) { setServerComparison(null); return }
    let cancelled = false
    setCompareLoading(true); setCompareError(''); setServerComparison(null)
    void compareScenarios(serverRefs.A.id, serverRefs.B.id)
      .then(value => { if (!cancelled) setServerComparison(value) })
      .catch(cause => { if (!cancelled) setCompareError(cause instanceof Error ? cause.message : 'Не удалось сравнить сценарии на сервере') })
      .finally(() => { if (!cancelled) setCompareLoading(false) })
    return () => { cancelled = true }
  }, [page, serverRefs.A?.id, serverRefs.B?.id])

  const reload = async () => {
    setLoading(true); setLoadError(''); setRestoreError(''); setBaseline(null)
    try {
      const loaded = await getData()
      setData(loaded)
      setComparison(current => forDataset(current, loaded.dataset_hash))
      const refs = refsForDataset(loadServerRefs(), loaded.dataset_hash)
      setServerRefs(refs)
      const slots = ['A', 'B'] as const
      const restored = await Promise.allSettled(slots.map(slot => refs[slot] ? getScenario(refs[slot].id) : Promise.resolve(null)))
      restored.forEach((entry, index) => {
        const slot = slots[index]
        if (entry.status === 'fulfilled') {
          const saved = entry.value
          if (saved?.dataset_hash === loaded.dataset_hash) setComparison(current => ({ ...current, [slot]: saved.result }))
        }
        if (entry.status === 'rejected') {
          const message = entry.reason instanceof Error ? entry.reason.message : 'Не удалось восстановить серверный сценарий'
          if (message.startsWith('API 404')) {
            setServerRefs(current => ({ ...current, [slot]: null }))
            setComparison(current => ({ ...current, [slot]: null }))
  ...
  const calculate = async (decisions: Decision[]) => {
    const revision = ++scenarioRevision.current
    setRunning(true); setRunError(''); setViolations([]); setAdvisor(null); setAdvisorError('')
    try {
      const response = await simulate(decisions)
      if (revision !== scenarioRevision.current) return
      if (!response.valid) { setViolations(response.violations.map(item => item.message)); setPage('decisions'); return }
  ...
  const run = async () => { if (canRun) await calculate(choices) }
  const applyRecommendation = async (decisions: Decision[]) => {
    setChoices(decisions)
    setResult(null)
    setPage('decisions')
    await calculate(decisions)
  }
  ...
  const saveResult = async (slot: Slot) => {
    if (!result || !data || result.dataset_hash !== data.dataset_hash) return
    setSavingSlot(slot); setSaveError('')
    try {
      const saved = await createScenario(scenarioName.trim() || `Сценарий ${slot}`, result.decisions)
      if (saved.dataset_hash !== data.dataset_hash) throw new Error('Датасет сервера изменился. Обновите страницу и пересчитайте сценарий.')
      setComparison(current => ({ ...forDataset(current, data.dataset_hash), [slot]: saved.result }))
      setServerRefs(current => ({ ...refsForDataset(current, data.dataset_hash), [slot]: { id: saved.id, name: saved.name, dataset_hash: saved.dataset_hash } }))
      setPage('compare')
    } catch (cause) { setSaveError(cause instanceof Error ? cause.message : 'Не удалось сохранить сценарий на сервере') }
