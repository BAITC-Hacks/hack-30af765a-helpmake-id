...
- Сохранение, список, получение, удаление и сравнение сценариев в PostgreSQL.
...
`/advisor/explain` принимает `{"simulation_result": <SimulationSuccess>}`. Для объяснения backend составляет проверенные факты о Score, районах, бюджете и критических показателях; AI выбирает по одному идентификатору факта для каждого раздела ответа. `/advisor/ask` добавляет `question` длиной от 1 до 1000 символов после удаления пробелов. Пустой, слишком длинный или некорректный вопрос возвращает `422`. Текст объяснения и ответа на вопрос собирается из подготовленных backend фактов, поэтому коды индикаторов вроде `S1` и `S2` допустимы, а новые числа или меры модель добавить не может.
...
Сценарии хранятся в PostgreSQL 17 через асинхронный SQLAlchemy Core и `asyncpg`. Схема создаётся только миграцией Alembic `20260923_01`; приложение не создаёт таблицы при запросе. `/ready` возвращает `503`, если БД недоступна или версия схемы не совпадает с ожидаемой. Миграции находятся в `backend/migrations/postgres/versions/`. Для новой схемы сначала измените SQLAlchemy-таблицу, затем создайте миграцию командой `make migration M="описание"`, проверьте её и примените `make migrate`.

В Docker Compose PostgreSQL сохраняет данные в volume `postgres_data`; сервис `migrate` завершает `alembic upgrade head` до запуска backend. На сервере PostgreSQL находится в приватной Docker-сети `akim-private` и volume `akim-postgres-data`; `deploy/deploy.sh` применяет миграции до переключения API. `DATABASE_URL` хранится только в защищённом серверном `deploy.env`. При переходе с SQLite на PostgreSQL сохранённых сценариев не было; старый SQLite volume оставлен на сервере для возможного отката приложения, но новым backend не используется. Перед будущими изменениями схемы делайте резервную копию PostgreSQL.
...
Сервер доступен на `http://127.0.0.1:8000`. `docker compose down` сохраняет данные; `down -v` удаляет volume PostgreSQL. Для запуска Uvicorn вне Docker сначала запустите PostgreSQL и экспортируйте `DATABASE_URL=postgresql+asyncpg://akim:<пароль>@127.0.0.1:5432/akim`, затем выполните `make migrate` и `make dev`.
...
| --- | --- |
| `POSTGRES_PASSWORD` | Обязательный URL-safe пароль PostgreSQL для Compose |
| `DATABASE_URL` | Обязательный URL `postgresql+asyncpg://...`; Compose задаёт его автоматически |
...
- PostgreSQL сохраняет сценарии между перезапусками; резервное копирование и мониторинг БД остаются задачей эксплуатации.
